...
"""
    ...
    def __init__(
        ...
        self.fast_test_command = f"cd {temp_dir} && clickhouse-test --hung-check --no-random-settings --no-random-merge-tree-settings --no-long --testname --shard --check-zookeeper-session --order random --report-logs-stats --fast-tests-only --no-stateful --jobs {nproc} -- '{{TEST}}' | ts '%Y-%m-%d %H:%M:%S' \
...
"""
    ...
    def terminate(self):
        ...
        for proc, pid_file, pid, run_path in (
            ...
            if proc and pid:
                if not Shell.check(
                    f"cd {run_path} && clickhouse stop --pid-path {Path(pid_file).parent}",
                    verbose=True,
                ):
    ...
    def prepare_logs(self, all=False):
        ...
        Shell.check("ulimit", verbose=True)
        Shell.check("find ./ci/tmp/ -name *core.*", verbose=True)
        if all:
